#include "Бинарное_дерево.h"	//подключаем файд заголовок
TreeElement* BinaryTree::findElement(int n)		//функция поиска элемента по информационному полю
{
	TreeElement* element = root;	//корень дерева
	int state = 0;
	while (true)
	{
...
TreeElement* BinaryTree::recursiveFindElement		//рекурсивная функция поиска элемента
(TreeElement* r, int n) {
...
void BinaryTree::add(int current_n, int new_n) {		//функция добавления узла
	if (root == nullptr) {
...
void BinaryTree::remove(int n) {		//функция удаления узла
	TreeElement* current = recursiveFindElement
	(root, n);
	if (current == nullptr) return;
...
void BinaryTree::recursiveRemove(int n, TreeElement* r)	//рекурсивная функция удаления узла
{
	if (r == nullptr) return;
